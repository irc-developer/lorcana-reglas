## ❓ Duda

- **Caso 1:** En juego y seco tengo [[02. Listado de Cartas/Set 1 - The First Chapter.md#Jafar - Keeper of Secrets|Jafar - Keeper of Secrets]] y una [[02. Listado de Cartas/Set 2 - Rise of the Floodborn.md#Queen Of Hearts - Sensing Weakness|Queen of Hearts - Sensing Weakness]]. ¿Qué fuerza tendrá mi Jafar cuando pasemos al Paso de daño por desafío?

- **Caso 2:** El número de cartas + 1 debido a que se robará antes de dicho paso de daños.

---

## ✅ Respuesta

**Caso 1:** No, las habilidades que indican "siempre que sea retado" se resuelven antes del [[01. Reglas/4. Acciones de turno (Turn Actions)/4.3. Jugar una carta (Play a Card)|Paso de daño por desafío]] con lo que estarías obligado a descartar primero la carta de tu mano, bajando su fuerza a uno.

**Caso 2:** El número de cartas + 1 debido a que se robará antes de dicho paso de daños.

---

## 📘 Fundamento en reglas

**Definición: Timing de habilidades "siempre que sea retado"**

Las habilidades que se disparan "siempre que este personaje sea retado" se resuelven ANTES del Paso de daño por desafío, no durante. Esto significa que cualquier modificación de fuerza ocurre antes de calcular el daño.

- [[01. Reglas/4. Acciones de turno (Turn Actions)/4.3. Jugar una carta (Play a Card)|4.3.6.2 Paso de daño por desafío]]
- [[01. Reglas/6. Habilidades, efectos y resolución (abilities, effects, and resolving)/6.2. Habilidades Disparadas (Triggered Abilities)|6.2 Habilidades disparadas y timing]]

---

## 🔄 Secuencia oficial

1. **Coste**: determinar el coste de la acción o habilidad que inicia la jugada.
2. **Objetivos**: fijar objetivos legales según el texto.
3. **Resolución**: resolver el texto en orden.
4. **Disparos**: añadir a la bolsa los triggers generados y resolver por prioridad.
5. **GSC**: realizar chequeo del estado del juego tras cada resolución.

---

## 🏷️ Tags

#challenge-damage-step #hand-size #strength

