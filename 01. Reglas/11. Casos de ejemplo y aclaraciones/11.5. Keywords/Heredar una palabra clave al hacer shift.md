## ❓ Duda

Tengo en zona de juego [[02. Listado de Cartas/Set 9 - Fabled.md#The Magic Feather|The Magic Feather]] aplicando Evasive a un personaje.  
Si juego un personaje con Shift sobre ese personaje, ¿conserva la palabra clave ganada por el efecto del objeto?

---

## ✅ Respuesta

Sí.  
Al hacer Shift, el nuevo personaje no hereda el texto impreso de la carta inferior, pero sí conserva los efectos que estaban aplicándose al personaje cuando entró mediante Shift. Por eso mantiene la palabra clave otorgada por [[02. Listado de Cartas/Set 9 - Fabled.md#The Magic Feather|The Magic Feather]] mientras el efecto siga activo.

---

## 📘 Fundamento en reglas

- [[8.10. Cambio (Shift)#8.10.1. Definición|8.10.1. Definición]]
- [[8.10. Cambio (Shift)#8.10.6. Acciones y efectos heredados|8.10.6. Acciones y efectos heredados]]
- [[5.1. Estados de las cartas (Card States)#5.1.3. Cambio de estado|5.1.3. Cambio de estado]]

---

## 🔄 Secuencia oficial

1. **Coste**: juegas el personaje usando su coste de Shift.
2. **Objetivos**: eliges la carta legal sobre la que se hará Shift.
3. **Resolución**: el personaje con Shift entra sobre el anterior y conserva los efectos aplicados al personaje base.
4. **Disparos**: se añaden a la bolsa los triggers que procedan por jugar carta o por cambios de estado.
5. **GSC**: se verifica estado del juego tras la resolución.

---

## 🏷️ Tags

#efectoscontinuos #evasive #shift